...
- **Forgejo** (`.forgejo/workflows/release.yml`) runs on every push to `main`. [`semantic-release`](https://semantic-release.gitbook.io/) analyses the commits, writes the new version to `VERSION`, updates `CHANGELOG.md`, commits `chore(release): vX.Y.Z`, and pushes the `vX.Y.Z` tag. `VERSION` is the single authoritative version source; `./build` reads it and stamps `CFBundleShortVersionString`. The release commit deliberately carries **no** `[skip ci]` marker: the `v*` tag points at that commit, and GitHub honours `[skip ci]` on the commit a tag references, which would silently skip the mirrored tag build. Loop prevention instead comes from `semantic-release` keying off the last git tag — when the job re-triggers on its own release commit it finds no commits since the fresh tag and exits, so each release costs one harmless no-op Forgejo run.
