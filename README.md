...
- **Forgejo** (`.forgejo/workflows/release.yml`) runs on every push to `main`. [`semantic-release`](https://semantic-release.gitbook.io/) analyses the commits, writes the new version to `VERSION`, updates `CHANGELOG.md`, commits `chore(release): vX.Y.Z`, and pushes the `vX.Y.Z` tag. `VERSION` is the single authoritative version source; `./build` reads it and stamps `CFBundleShortVersionString`. The release commit message is deliberately **subject-only** (`chore(release): vX.Y.Z`, no notes body): the `v*` tag points at that commit, and GitHub honours skip-ci markers (`[skip ci]`, `[ci skip]`, …) on the commit a tag references, which would silently skip the mirrored tag build. Keeping the notes out of the commit body prevents a prior commit subject that happens to quote such a marker from leaking into the tagged commit — the notes still reach `CHANGELOG.md` and the GitHub release. Loop prevention does not rely on skip-ci at all: `semantic-release` keys off the last git tag, so when the job re-triggers on its own release commit it finds no commits since the fresh tag and exits — one harmless no-op Forgejo run per release.
